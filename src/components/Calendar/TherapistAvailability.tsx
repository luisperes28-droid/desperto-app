...
import { Calendar, Clock, Plus, X, Save, AlertCircle, Loader2 } from 'lucide-react';
...
export function TherapistAvailabilityManager({ therapistId, onClose }: TherapistAvailabilityProps) {
  const { therapists, setTherapists, therapistAvailability, setTherapistAvailability, refreshData } = useApp();
  ...
  const handleSave = async () => {
    ...
    const success = await SupabaseDataService.saveTherapistAvailability(
      userId,
      therapistId,
      availability
    );
    ...
    if (success) {
      setTherapists(prev => prev.map(t =>
        t.id === therapistId ? { ...t, availability } : t
      ));
      ...
      setSaveError('Erro ao guardar. Verifique as suas permissoes.');
    }
      ...
      <div className="bg-white rounded-xl max-w-4xl w-full max-h-[95vh] flex flex-col">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600">Configure os seus horarios disponiveis</p>
              </div>
            </div>
        ...
        <div className="border-b border-gray-200 flex-shrink-0">
          <nav className="flex space-x-8 px-6">
            {[
              { id: 'schedule', label: 'Horario Semanal', icon: Clock },
              { id: 'blocked', label: 'Datas Bloqueadas', icon: X },
              { id: 'settings', label: 'Configuracoes', icon: Calendar }
            ].map((tab) => {
              const Icon = tab.icon;
              return (
                <button
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {activeTab === 'schedule' && (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Horario de Trabalho</h3>
                  ...
                  {availability.breaks.map((break_, index) => (
                    <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <input
                        ...
                        className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                      />
                      <button onClick={() => handleRemoveBreak(index)} className="text-red-600 hover:text-red-800">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
          ...
          {activeTab === 'blocked' && (
            <div className="space-y-6">
                  ...
                  {availability.blockedDates.length === 0 ? (
                    ...
                    availability.blockedDates.map((date, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-red-50 border border-red-200 rounded-lg">
                        ...
                        <button onClick={() => handleRemoveBlockedDate(index)} className="text-red-600 hover:text-red-800">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ))
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Horarios Especiais Disponiveis</h3>
                ...
                <div className="space-y-2">
                  {availability.customSchedule.map((schedule, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-primary-50 border border-primary-200 rounded-lg">
                      ...
                      <button onClick={() => handleRemoveCustomSchedule(index)} className="text-red-600 hover:text-red-800">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Agendamento Antecipado Maximo (dias)
                  </label>
                  <input
                    type="number"
                    min="1"
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="font-medium text-green-900">Resumo da Configuracao</span>
                </div>
                <div className="text-sm text-green-800 space-y-1">
                  <p>Dias de trabalho: {availability.workingDays.map(d => dayNames[d]).join(', ')}</p>
                  <p>Horario: {availability.workingHours.start} - {availability.workingHours.end}</p>
                  <p>Pausas: {availability.breaks.length} configuradas</p>
                  <p>Datas bloqueadas: {availability.blockedDates.length}</p>
                  <p>Horarios especiais: {availability.customSchedule.length}</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-6 border-t border-gray-200 flex-shrink-0 bg-gray-50">
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center space-x-2 px-8 py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 font-medium shadow-md disabled:opacity-50"
            >
              {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
              <span>{saving ? 'A guardar...' : 'Guardar Disponibilidade'}</span>
            </button>
          </div>
        </div>
      </div>
